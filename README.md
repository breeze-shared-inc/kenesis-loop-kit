...
> 手順2〜4は、Claude Code起動後に `/setup` で対話的に実行できます（.gitignoreプリセットの選択・Obsidianの案内・プロジェクト略称の確定・SPEC有無に応じた次の一手の案内）。`/setup` は冪等で、導入済みプロジェクトの現在地診断としても再実行できます。
...
| コマンド | 用途 | 使用例 |
|---|---|---|
| `/setup` | プロジェクトの初期セットアップと現在地診断 | `/setup` |
...
2. Claude Codeを起動して `/setup` を実行する（.gitignoreプリセットの選択・プロジェクト略称の確定・`/spec-interview` など次の一手への誘導まで対話的に進む）
3. 案内に従い `/spec-interview` → `/interrogate-spec` → `/wireframe-gen` で前工程を整備する
4. Obsidianを使う場合は`tickets/`をvaultとして開き、`docs/obsidian-setup.md`に従って初期設定をする（任意）
