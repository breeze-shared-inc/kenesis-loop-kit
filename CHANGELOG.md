...
- `.claude/settings.json` の allow に `echo` / `sed -n`（読み取り専用）を再追加（probe/テスト後始末の複合コマンドを自動許可するため）。環境固有の `.venv/bin/python` / `rm -f` ルールは `settings.local.json` へ分離
...
- `.claude/settings.json` の allow から `awk`（ファイル書き込みをWrite権限に一元化）。※ `echo` / `sed -n` は probe/テスト用途で後に再追加（上記 Added 参照）
