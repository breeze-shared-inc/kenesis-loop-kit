...
3. チケットごとに `claude -p "/start-loop {ID}" --permission-mode acceptEdits --max-turns 200` を新規セッションとして起動する（標準入出力は継承・ターミナルでライブ監視できる）。駆動プロンプトには「このセッションで処理するのは {ID} の1件のみ」を明示し、バッチの事前承認はセッションへ**与えない**
