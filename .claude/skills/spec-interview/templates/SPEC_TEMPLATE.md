...
<!-- ワイヤーフレーム作成後に画面の過不足へ気づいた場合も、版を上げて「6. 画面一覧・画面遷移」へ反映すること -->

| 版数 | 更新日 | 更新者 | 変更概要 |
|---|---|---|---|
| v1.0 | 2026-07-06 | msrn-t | 初版作成 |
| v1.1 | 2026-07-07 | msrn-t | ワイヤーフレーム作成に伴い画面一覧を改訂(例) |
...
<!-- Must項目には正常系だけでなく「失敗時・異常時の挙動」も必ず記述する(詳細な方針は「7. エラーハンドリング方針」を参照) -->
...
<!-- 本セクションは「画面の存在と役割」を定義する。レイアウト・配置・ビジュアルの詳細はワイヤーフレーム側に記載し、本書には書かない(二重管理による乖離を防ぐため) -->
<!-- 画面にはID(SCR-xxx)を振り、IDは欠番が出ても再利用しない -->
...
<!-- UIを持たないプロジェクト(API・バッチ等)の場合は、本セクションに「該当なし」と明記すること -->
...
- 入力バリデーション: (例: すべての入力はサーバーサイドで検証する。クライアント検証は補助とする)
- 認証失敗時: (例: 5回連続失敗で15分間アカウントをロックする)
- 外部サービス障害時: (例: メール送信失敗時は3回までリトライし、失敗をログに記録する)
- 想定外エラー: (例: ユーザーには汎用エラー画面を表示し、詳細はサーバーログにのみ出力する)
...
<!-- シークレット(APIキー・接続文字列など)の値は本文書に記載せず、環境変数名のみを記載すること(CLAUDE.mdセキュリティ規約に準拠) -->
...
<!-- まだ決まっていない事項をIDを振って列挙する。決まり次第、内容を該当セクションへ反映し、この表のステータスを「決定済」に更新する -->
...
<!-- ワイヤーフレームは「6. 画面一覧・画面遷移」から参照するため、本セクションには記載不要 -->
