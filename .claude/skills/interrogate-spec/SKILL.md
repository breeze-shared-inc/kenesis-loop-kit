...
1. answered / open / deferred / invalidated の件数と次バッチの見込み(1〜2行)。
   open または presented が残る場合は「続きは `/interrogate-spec` の再実行」を添える
2. 高rank質問が集中しているSPEC節の上位(1行)。特定の節に質問が密集している場合、
   個別回答よりその節自体の書き直し(責務過多・抽象度不整合の解消)が有効である
   可能性を一言添える。
3. 次の一手の案内: open と presented がともに0件の場合**のみ**、次の骨子で案内する。
   「現時点で提示可能な質問はありません(セッション外の検証待ち deferred が n 件、
   うち high rank が k 件あります。先へ進むかはこの残数を見て判断してください)。
   次の一手の判定は `/setup` の現在地診断が正です。典型は `/wireframe-gen`
   (SPEC §6 画面一覧が「該当なし」のUIなしプロジェクトは `/plan-tickets`)。」
   deferred の件数と high rank の有無の併記は省略しない(前進判断はリスク受容の
   意思決定であり、判断材料と判断主体(人間)を必ず明示する)。「完了しました」とは
   宣言しない(deferred は T5 で open へ戻り得るため、報告するのは「現時点で提示
   可能な質問がない」というスナップショット事実のみ)。ルーティングの正は
   `.claude/commands/setup.md` 手順5であり、次工程の実行はこのスキルの責務外
   (案内のみ行い、起動しない)。
