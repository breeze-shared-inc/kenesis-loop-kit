...
AWK_SAFE_PROGRAM_CHARS = frozenset(
    ...
    # 誤denyになる（設計書 §6 R33 の列挙外）。固定点は INV-AWKLEX-13
    " \t\n"
    '$(){}[],;:?!~=<>+-*/%^&."#'
)
...
def awk_strip_literals(text):
    ...
    **除去しないことは常に deny 方向（安全）／除去しすぎることだけが
    ...
    - **AWL-2 正規表現 `/…/`**: 直前の非空白文字が AWK_OPERAND_END_CHARS に
      あれば除算とみなし**除去しない**。候補スパンが `;` か改行を含む場合も
      除去しない（内容が判定対象に残る＝deny 方向）。
    ...
    **P9（安全性主張の様式）に従った本関数の主張:**
      ...
      - 向き: over-removal は **allow（危険）**／under-removal は
        **deny（安全）**。
      - 検出器: T-C9a〜T-C9h・T-AW7h（テスト）／設計書 §4-9 の **V-15**
        （実機 gawk との二方向突き合わせ・§3-9-3 の全13サブケース）。
    ...
    while i < n:
        ...
        if char == '"':  # AWL-1（文字列）
            ...
            prev, i = '"', j + 1
            continue
        if char == "/" and prev not in AWK_OPERAND_END_CHARS:  # AWL-2
            j, closed = i + 1, False
            while j < n:
                if text[j] == "\\":
                    j += 2
