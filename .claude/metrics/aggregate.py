...
依存: Python3標準ライブラリ + .claude/hooks/_ticket_lib.py
（イベントログの読み込みを record_metrics.py / check_loop_integrity.py と共有する）。
"""
import datetime
import os
import sys
...
def load_events(path, filter_str):
    """イベントログを読み、FILTER 指定時は ticket / project 部分一致で絞り込む。
    行の読み込み・不正行スキップは _ticket_lib と共有する。"""
    events = lib.load_events(path)
    if not filter_str:
        return events
    needle = filter_str.lower()
    return [
        ev for ev in events
        if needle in ("%s %s" % (ev.get("ticket", ""),
                                 ev.get("project", ""))).lower()
    ]
