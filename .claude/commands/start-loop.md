...
複数チケットを連続実行したい場合は外部駆動バッチ（1チケット=1セッション）を使う — `/batch-loop {ID-1} {ID-2} ...` がプリフライト検証と、人間がターミナルで実行する `scripts/batch_loop.py` のコマンドラインを案内する（運用ガイドは `docs/batch-loop.md` を参照）。
