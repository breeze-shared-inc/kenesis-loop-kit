...
本ファイルは全体像のインデックスと、複数ファイルから参照される共通定義（ステータス定義・チケットID採番・ブランチ戦略）を持ちます。個別ポリシーの詳細は「ポリシー管理の原則」の表が示す定義ファイルを正とします。
...
運用ルールの詳細（ブランチ作成・コミットメッセージ規約 `[{チケットID}] {変更内容の要約}`・WIPコミット・testerのテストコミット・コミット前の機密確認・`git push` は人間のみ）は `.claude/agents/implementer.md` / `tester.md` の「Git Rules」を正とする。
...
| Git運用規約（ブランチ・コミット・機密確認） | 本ファイル ブランチ戦略 + implementer.md / tester.md Git Rules | implementer / tester | - |
...
新しいポリシーを追加する際は、このテーブルを更新し、定義ファイル・転記先への反映まで完了させること。
